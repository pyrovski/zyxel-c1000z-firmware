...
#if 1 // __CTL__, queue counters
static int proc_get_tx_counter(char *page, char **start, off_t off, int cnt, int *eof, void *data)
{
  int r = 0;

  UINT32 i, ulDropped[ENET_TX_EGRESS_QUEUES_MAX] = {0};
  UINT64 ulSent[ENET_TX_EGRESS_QUEUES_MAX] = {0}, ulSentBytes[ENET_TX_EGRESS_QUEUES_MAX] = {0};
  volatile fap4kePsm_global_t * pPsmGblfap0 = (volatile fap4kePsm_global_t *)pHostPsmGbl(PKTDMA_US_FAP_INDEX);
  //in 63268, WAN to LAN traffic is forwarded by FAP1
  volatile fap4kePsm_global_t * pPsmGblfap1 = (volatile fap4kePsm_global_t *)pHostPsmGbl(PKTDMA_DS_FAP_INDEX);

  for (i = 0; i < ENET_TX_EGRESS_QUEUES_MAX; i++)
  {
	ulSent[i] += pPsmGblfap0->ethSent[i];
	ulSentBytes[i] += pPsmGblfap0->ethSentBytes[i];
	ulDropped[i] += pPsmGblfap0->ethDropped[i];
  }

  for (i = 0; i < ENET_TX_EGRESS_QUEUES_MAX; i++)
  {
	ulSent[i] += pPsmGblfap1->ethSent[i];
	ulSentBytes[i] += pPsmGblfap1->ethSentBytes[i];
	ulDropped[i] += pPsmGblfap1->ethDropped[i];
  }

  for (i = 0;i < ENET_TX_EGRESS_QUEUES_MAX; i++)
  {
  	r += sprintf(page + r, "Priority: %lu Sent: %llu SentBytes: %llu Dropped: %lu\n",
			  i, ulSent[i] , ulSentBytes[i], ulDropped[i]);
  }
...
#if 1 // __CTL__, queue counters
  sprintf(tmp, "driver/%s/txcounter", dev->name);
  remove_proc_entry(tmp, NULL);
#endif
...
#if 1 // __CTL__, queue counters
  sprintf(tmp, "driver/%s/txcounter", dev->name);
  create_proc_read_entry(tmp, 0, NULL, proc_get_tx_counter, p);
#endif
