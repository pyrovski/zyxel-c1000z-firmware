...
//CSP#512462, ethctl can't get the speed mode of phy with address less than 0x10. KuanJung
 ...
 */
#if 1//LEO
static int portflag[8]={0,0,0,0,0,0,0,0}; //leo
#endif
...
//CSP#512462, ethctl can't get the speed mode of phy with address less than 0x10. KuanJung
            swPort = port_id_from_dev(dev);
            ethsw_phy_rreg(ETHSW_PHY_GET_PHYID(swPort), mii->reg_num & 0x1f, (uint16 *)&mii->val_out);
#else
...
//CSP#512462, ethctl can't get the speed mode of phy with address less than 0x10. KuanJung
            swPort = port_id_from_dev(dev);
            ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(swPort), mii->reg_num & 0x1f, (uint16 *)&mii->val_in);
#else
