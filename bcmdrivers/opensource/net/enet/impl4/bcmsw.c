...
void ethsw_init_hw(int unit, uint32_t portMap,  int wanPortMap, int is6829)
{
  ...
  for (i = 0; i < NUM_RGMII_PORTS; i++) {
   ...
   ethsw_wreg(PAGE_MANAGEMENT, REG_MIRROR_CAPTURE_CTRL,(uint8 *)&v16, 2);
    ...
    //Leo
    //enable ethernet@wirespeed of all GE port
    x16 = 0x81f7;
    for (ii = 4; ii < 8; ii++)
      ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(ii), 0x18, &x16);

    //JhenYang, ethernet@wirespeed for internal Giga phy.
    x16 = 0x8077;
    ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(3), 0x18, &x16);

    //enable Giga speed for internal Giga phy.
    x16 = 0x0200;
    ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(3), 0x09, &x16);
#endif
