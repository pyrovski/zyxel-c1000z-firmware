    ...
    for (i = 0; i < (TOTAL_SWITCH_PORTS - 1); i++) {
        ...
        if ((pVnetDev0->EnetInfo[0].sw.port_map & (1U<<i)) != 0) {
            ...
            if (IsExtPhyId(phy_id)) {
                ...
                if ((v16 & BCM_PHYID_M) == (BCM54610_PHYID2 & BCM_PHYID_M) || (v16 & BCM_PHYID_M) == (BCM50612_PHYID2& BCM_PHYID_M)) {
#else
                if ((v16 & BCM_PHYID_M) == (BCM54610_PHYID2 & BCM_PHYID_M)) {
                    ...
                    ethsw_phy_wreg(phy_id, MII_REGISTER_1C, &v16);
                    ethsw_phy_rreg(phy_id, MII_REGISTER_1C, &v16);
#if 1 // __CTL__, KuanJung. set the RGMII mode to 2.5V
					v16 &= ~(LOM_LED_MODE);
					v16 |= (1 << 3);
					v16 &= ~(1 << 4);
#else
...
#if 1 // __CTL__, KuanJung. Disable EEE from CSP 529676
...
/* Clause 45 register write for integrated switch */
void ethsw_phyport_c45_wreg(int port, int regg, int regr, uint16 *pdata16) {
   uint16 data16;
   data16 = regg;
   ethsw_phyport_wreg(port, 0x0d, &data16);
   data16 = regr;
   ethsw_phyport_wreg(port, 0x0e, &data16);
   data16 = 0x4000 | regg;
   ethsw_phyport_wreg(port, 0x0d, &data16);
   ...
   if ((unit == 0) && IsPhyConnected(phy_id) && IsExtPhyId(phy_id)) {
      ...
      if (data16 == 0x0362) {
         ...
         if ((data16 == 0x5e61) || (data16 == 0x5e62) || (data16 == 0x5e6a) || (data16 == 0x5f6a) ||
            ...
            if (!enable) {
               data16 = 0;
               ethsw_phyport_c45_wreg(port, 7, 0x3c, &data16);
               ethsw_phyport_c45_wreg(port, 7, 0x803d, &data16);
            }
      }
      }
   }
