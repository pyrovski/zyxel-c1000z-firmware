...
#if 1 // __CTL__, KJ
static void bcmQosRemoveLease(char *cmd);
#endif
...
void bcmSetQosRule(char action, char *command, u_int32_t leaseip)
{
   ...
   if (ptokenstart == NULL)
   {
   	cmsLog_error("can't find [ in qos rule");
	return;
   }
#endif
   ip.s_addr   = leaseip;
...
#if 1 // __CTL__, KJ
//clean the lease ip of some qos rule
void bcmQosRemoveLease(char *cmd)
{
   struct optioncmd *pnode, *pprevnode;
   int i;
