...
struct dhcpOfferedAddr *sendACK(struct dhcpMessage *oldpacket, u_int32_t yiaddr)
{
        ...
        char *lease_time, *vendorid, *userclsid, *clientid;
        char length = 0;
        ...
        clientid = (char *)get_option(oldpacket, DHCP_CLIENT_ID);
...
#if 1 //__CTLK__, Thief
		if( clientid != NULL){
            char *pclientid = offerlist->clientid;
			int i, tmp_clientid;
			length = *(clientid - 1);
            for(i = 0; i < length && i < 127; i++)
            {
                tmp_clientid = *(clientid+i);
                tmp_clientid &= 0x000000ff;
                sprintf(pclientid, "%02x", tmp_clientid);
				pclientid +=2;
            }
		}
#endif
