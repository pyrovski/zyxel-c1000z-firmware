...
static void init_packet(struct dhcpMessage *packet, char type)
{
	...
	if (client_config.domainName) add_option_string(packet->options, (unsigned char *)(client_config.domainName));
#endif
	// brcm
...
#if 1//__CTLK__,JhenYang
   if(en_VAH == 0)
      syslog(LOG_CRIT,"DHCP send discover packet\n");
#endif
...
#if 1//__CTLK__,JhenYang
	if(en_VAH == 0)
		syslog(LOG_CRIT,"DHCP send request packet\n");
#endif
	return raw_packet(&packet, INADDR_ANY, CLIENT_PORT, INADDR_BROADCAST, 
				SERVER_PORT, MAC_BCAST_ADDR, client_config.ifindex);
}
...
int send_renew(unsigned long xid, unsigned long server, unsigned long ciaddr)
{
   ...
   if(en_VAH == 0)
	{
      if(server)
         syslog(LOG_CRIT,"DHCP send renew packet\n");
      else
         syslog(LOG_CRIT,"DHCP send rebinding packet\n");
   }
#endif
...
int send_release(unsigned long server, unsigned long ciaddr)
{
	...
	add_simple_option(packet.options, DHCP_SERVER_ID, ntohl(server));
#if 1//__CTLK__,JhenYang
	syslog(LOG_CRIT,"DHCP release IP address\n");
#endif
